...
public class ForecastPWS2PartnerControlador {
	
	
	public String generaForecastPWS2Partner(String coFcst, String noNave, String token) throws SQLException {

		List<ForecastPWS2Partner> forecast;
		List<ForecastPWS2PartnerCargo> cargos;

		String fileName = null;

		forecast = obtieneForecastPWS2Partner(coFcst, token);

		if (!forecast.isEmpty()) {

			cargos = obtieneForecastPWS2PartnerCargo(coFcst, token);

			fileName = generaExcelForecastPWS2Partner(forecast, noNave, coFcst, cargos);
	...
	private List<ForecastPWS2Partner> obtieneForecastPWS2Partner(String coFcst, String token) throws SQLException {

		return (new ForecastDao()).obtieneForecastPWS2Partner(coFcst, token);

	}
	
	

	private List<ForecastPWS2PartnerCargo> obtieneForecastPWS2PartnerCargo(String coFcst, String token) throws SQLException {

		return (new ForecastDao()).obtieneForecastPWS2PartnerCargo(coFcst, token);

	}
	
	
	private String generaExcelForecastPWS2Partner(List<ForecastPWS2Partner> data, String alNave, String coFcst,
			List<ForecastPWS2PartnerCargo> cargos) {
		
		String fileName = "ForecastPWS2Partner_" + Instant.now().toEpochMilli() + ".xlsx";
		...
		try {
			...
			for (ForecastPWS2Partner forecast : data) {
			...
			List<ForecastPWS2PartnerCargo> rh = cargos.stream().filter(x -> x.getTipo().equals("RH"))
					.collect(Collectors.toList());
			List<ForecastPWS2PartnerCargo> dg = cargos.stream().filter(x -> x.getTipo().equals("DG"))
					.collect(Collectors.toList());
	...
	private XSSFCellStyle creaEstilosTitulo(XSSFWorkbook wb) {
		...
		// COLOR DE FONDO
		XSSFColor colorFondo = new XSSFColor(new Color(69, 178, 69));
	...
	private XSSFCellStyle creaEstilosTituloDG(XSSFWorkbook wb) {
		...
		// COLOR DE FONDO
		XSSFColor colorFondo = new XSSFColor(new Color(21, 186, 243));
		style.setFillPattern(FillPatternType.SOLID_FOREGROUND);
		style.setFillForegroundColor(colorFondo);
