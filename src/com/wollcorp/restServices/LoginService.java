	...
	public Response servicioLogin(@HeaderParam("authorization") String auth) {
		...
		if (auth != null) {
			...
			if (datosLogin.getConexion() != null) {
				...
				return Response.status(Response.Status.OK).entity(datosLogin).build();

			} else if (datosLogin.getError() != null) {

				return Response.status(Response.Status.UNAUTHORIZED).entity(datosLogin).build();
		...
		return Response.status(Response.Status.BAD_REQUEST).entity(datosLogin).build();
